...
Place a heavy fluid above a light one in a gravitational field, and the interface erupts into complexity. Fingers plunge, bubbles rise, and small perturbations grow into chaotic, multiscale mixing. RTI is hard for machine learning because this chaos does not just make individual trajectories difficult to predict; it exposes a deeper simulation-to-laboratory gap. Idealized DNS usually begins from clean, short-wavelength perturbations and settles near α ≈ 0.02-0.03, while laboratory experiments carry apparatus-driven, large-scale initial structure and reach α ≈ 0.06-0.07. This makes RTI a sharp test of whether a foundation model has learned more than the DNS regime it was finetuned on. If the same DNS-specialized model gives DNS-like growth from DNS frames but enters the experimental growth band from experimental frames at zero-shot, then the input initial frames alone have moved it across the sim-to-real divide.
