...
Both datasets are generated by a compressible flow solver, but while "Near" (center) is selected to be physically very similar to the incompressible Navier-Stokes data in the training set (left), "Far" is generated in a different flow regime that exhibits wildly different behavior across scales. In both cases, the differences in solver and resolution compared to the training data still makes for large diff
